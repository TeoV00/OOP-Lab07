...
public class Sport2SocialNetworkUserImpl<U extends User> extends SocialNetworkUserImpl<U> {
     ...
     */
    private final Set<Sport> followedSport = new TreeSet<>();
    /**
    ...
    public boolean likesSport(final Sport s) {
        return followedSport.contains(s);
    }
     ...
     */
    /*
     * That could be resolved with lambda and stream
     * */
    public Set<Sport> getIndividualSports() {
        Set<Sport> indSports = new TreeSet<>();
        for(Sport sport : this.followedSport) {
    ...
    public Set<Sport> getSportPracticedInPlace(Place p) {
        Set<Sport> inPlaceSports = new TreeSet<>();
        for(Sport sport : this.followedSport) {
            if (sport.getPlace().equals(p)) {
                inPlaceSports.add(sport);
            }
        }
